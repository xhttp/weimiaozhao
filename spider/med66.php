...
$base_url = 'http://www.med66.com';

// 抓取某个指定的分类，全量就将这里设为空
$crawler_category = '';

$category_conf = array(array('url' => 'http://www.med66.com/web/kouqiangyishi/', 'name' => '口腔医学知识')
                       );
...
Host:www.med66.com
Referer:http://www.med66.com/web/kouqiangyishi/
Pragma:no-cache
...
foreach($category_conf as $category_item) {
    ...
    echo $list_url, "\n";    
        ...
        if(preg_match_all('/·<a href=([a-zA-Z0-9\/\.]+)>/isU', $list_body, $list_row_array)) {
...
//print_r($list_row_array);exit();
    ...
    echo $detail_url, "\n";
                ...
                //
                if($page_no == 1) {
                    if(preg_match('/<h2>(.*)<\/h2>/isU', $detail_body, $detail_title_array)) {
                        $title = trim($detail_title_array[1]);
                        $title = str_replace("\r", "", $title);
                        $title = str_replace("\n", "", $title);
                    }

                    if(preg_match('/<div class="mark c_hui">([\d]{4}-[\d]{2}-[\d]{2} [\d]{2}:[\d]{2})/isU', $detail_body, $detail_time_array)) {
                        $pub_time = $detail_time_array[1];
}
                }
                if(preg_match('/<div id="fontzoom">(.*)<\/div>/isU', $detail_body, $detail_body_array)) {
...
//exit();
        ...
        if(preg_match('/<a href="([a-zA-Z0-9]+\.shtm)" target="_self">下一页/isU', $list_body, $list_next_array))    {
