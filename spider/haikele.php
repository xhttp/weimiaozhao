...
$category_conf = array(array('url' => 'http://www.haikele.com/sfcwzlb.aspx?CategoryID=Y', 'name' => '营养课堂'),
                       array('url' => 'http://www.haikele.com/sfcwzlb.aspx?CategoryID=S', 'name' => '养生知识'),
                       array('url' => 'http://www.haikele.com/sfcwzlb.aspx?CategoryID=J', 'name' => '健康时事'),
                       array('url' => 'http://www.haikele.com/sfcwzlb.aspx?CategoryID=C', 'name' => '餐桌揭秘')
                       );
...
foreach($category_conf as $category_item) {
    ...
    echo $list_url, "\n";    
        ...
        if(preg_match_all('/<a id="(ctl00_Body_rep_sfcwzb_ctl[\d]+_Title)" href="(ArticleDetail\.aspx\?BlogID=[\d]+)" target="_blank">(.*)<\/a><\/span>/isU', $list_body, $list_row_array)) {
...
//print_r($list_row_array);exit();
    ...
    echo $detail_url, "\n";
                ...
                if(preg_match('/<div class="cArticle">(.*)<br \/> <br \/>/isU', $detail_body, $detail_body_array)) {
        ...
        if(preg_match('/<a href="(sfcwzlb\.aspx\?CategoryID=[A-Z]+&amp;page=[\d+])" style="margin-right:5px;color:#ccc;">下一页/isU', $list_body, $list_next_array))    {
            $list_url = $list_base_url.str_replace('&amp;', '&', $list_next_array[1]);        
            $list_page_no++;
        } else {
            break;
        }
