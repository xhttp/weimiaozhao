...
$base_url = 'http://www.ys137.com';

// 抓取某个指定的分类，全量就将这里设为空
$crawler_category = '';

$category_conf = array(array('url' => 'http://www.ys137.com/ydys/tyjs/', 'name' => '体育健身'),
                       array('url' => 'http://www.ys137.com/ydys/hwyd/', 'name' => '户外活动'),
                       array('url' => 'http://www.ys137.com/ydys/zhwsh/', 'name' => '中华武术'),
                       array('url' => 'http://www.ys137.com/ydys/yjyd/', 'name' => '瑜伽运动'),
                       array('url' => 'http://www.ys137.com/ydys/jfshsh/', 'name' => '减肥塑身'),
                       array('url' => 'http://www.ys137.com/ydys/guangchangwu/', 'name' => '广场舞'),
                       );
...
foreach($category_conf as $category_item) {
    ...
    echo $list_url, "\n";    
        ...
        if(preg_match_all('/<h2><a href="(\/ydys\/[0-9]+\.html)"/isU', $list_body, $list_row_array)) {
    ...
    echo $detail_url, "\n";
                ...
                if($page_no == 1) {
                    ...
                    if(preg_match('/发表时间：([\d]{4}\-[\d]{2}\-[\d]{2} [\d]{2}:[\d]{2})/', $detail_body, $detail_time_array)) {
                        $pub_time = $detail_time_array[1];
                    }
                }
                if(preg_match('/<table cellspacing="0" cellpadding="0"><tr><td>(.*)<\/td><\/tr><\/table>/isU', $detail_body, $detail_body_array)) {
                    $content .= $detail_body_array[1];
                }
                if(preg_match('/<a target=\'_self\' href=\'([\d]+_[\d]+\.html)\'>下一页</isU', $detail_body, $detail_url_array)) {
                    $detail_url = $base_url.'/ydys/'.$detail_url_array[1];
                    $page_no++;
                } else {
                    // 该详情页是否抓取过
        ...
        if(preg_match('/<li><a href=\'(list_[\d]+_[\d]+\.html)\'>下一页/isU', $list_body, $list_next_array))    {
