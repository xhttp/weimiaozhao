...
$base_url = 'http://auto.cnfol.com/chezhuketang/';

// 抓取某个指定的分类，全量就将这里设为空
$crawler_category = '';

$category_conf = array(array('url' => 'http://auto.cnfol.com/chezhuketang/', 'name' => '新车主课堂'),
                       array('url' => 'http://auto.cnfol.com/qicheyongpin/', 'name' => '汽车用品'),
                       array('url' => 'http://auto.cnfol.com/weixiubaoyang/', 'name' => '维修保养'),
                       array('url' => 'http://auto.cnfol.com/zijialvyou/', 'name' => '自驾旅游')
                       );
...
foreach($category_conf as $category_item) {
    ...
    echo $list_url, "\n";    
        ...
        if(preg_match_all('/<a title="(.*)" target="_blank" href="(http:\/\/auto\.cnfol\.com\/[a-zA-Z]+\/[\d]+\/[\d]+\.shtml)" class="Fl">/isU', $list_body, $list_row_array)) {
...
//print_r($list_row_array);exit();
    ...
    echo $detail_url, "\n";
                ...
                //
                if($page_no == 1) {
                    if(preg_match('/<div class="wzyL1">(.*)<\/div>/isU', $detail_body, $detail_title_array)) {
                        $title = trim($detail_title_array[1]);
                        $title = str_replace("\r", "", $title);
                        $title = str_replace("\n", "", $title);
                    }
                    if(preg_match('/<span id="pubtime_baidu">([\d]{4}-[\d]{2}-[\d]{2} [\d]{2}:[\d]{2}:[\d]{2})/isU', $detail_body, $detail_time_array)) {
                        $pub_time = $detail_time_array[1];
                    }
                }
                if(preg_match('/<div id="__content" class="wzyL3">(.*)<\/div>[\s]+<div class="wzyL5" id="page">/isU', $detail_body, $detail_body_array)) {
...
//exit();
        ...
        if(preg_match('/<a class="BtPA" href="(.*)">下一页<\/a><\/div>/isU', $list_body, $list_next_array))    {
