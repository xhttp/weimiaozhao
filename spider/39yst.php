...
$category_conf = array(array('url' => 'http://www.39yst.com/weixin/', 'name' => '微信'),
                       );
...
foreach($category_conf as $category_item) {
    ...
    echo $list_url, "\n";    
        ...
        if(preg_match_all('/<p class="item_tit"><a alt=\'(.*)\' title=\'(.*)\' href=\'(http:\/\/www\.39yst\.com\/weixin\/[\d\/]+\.shtml)\' target="_blank">/isU', $list_body, $list_row_array)) {
            $row_num = count($list_row_array[3]);
        }
        ...
        // 检测该列表页下的详情页是否被抓取过
        foreach($list_row_array[3] as $detail_url) {
            // 详情页第一页 url
    ...
    echo $detail_url, "\n";
                ...
                if($page_no == 1) {
                    ...
                    if(preg_match('/<span>([\d]{4}\-[\d]{2}\-[\d]{2} [\d]{2}:[\d]{2})<\/span>/', $detail_body, $detail_time_array)) {
                        $pub_time = $detail_time_array[1];
                    }
                }
                if(preg_match('/<div id="articleContent" class="article-content">(.*)<\/div>/isU', $detail_body, $detail_body_array)) {
        ...
        if(preg_match('/<a href="(\/weixin\/list_[\d]+_[\d]+\.shtml)" class="next" target="_self">下一页<\/a>/isU', $list_body, $list_next_array))    {
