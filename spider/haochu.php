...
$category_conf = array(array('url' => 'http://www.haochu.com/baike/26', 'name' => '饮食资讯'),
            array('url' => 'http://www.haochu.com/baike/27', 'name' => '美容瘦身'), 
            array('url' => 'http://www.haochu.com/baike/25', 'name' => '饮食常识'), 
            array('url' => 'http://www.haochu.com/baike/28', 'name' => '食疗养生'), 
            array('url' => 'http://www.haochu.com/baike/29', 'name' => '养生妙方'), 
            array('url' => 'http://www.haochu.com/jiankang/22', 'name' => '女性健康'), 
            array('url' => 'http://www.haochu.com/jiankang/21', 'name' => '男性健康'), 
            array('url' => 'http://www.haochu.com/jiankang/19', 'name' => '母婴健康'), 
            array('url' => 'http://www.haochu.com/jiankang/23', 'name' => '老年健康'),
            array('url' => 'http://www.haochu.com/jiankang/20', 'name' => '家庭健康'),
            array('url' => 'http://www.haochu.com/jiankang/87', 'name' => '情感八卦'),
            array('url' => 'http://www.haochu.com/ketang', 'name' => '美食课堂'));
...
foreach($category_conf as $category_item) {
    ...
    echo $list_url, "\n";    
        ...
        if(preg_match_all('/<h2 class="v1_con1"><a href="(\/[a-z]+\/[\d]+\.html)">/isU', $list_body, $list_row_array)) {
            $row_num = count($list_row_array[1]);
        }
        // ketang
        elseif(preg_match_all('/<div class="tang_img_name clearfix"><span><a href="(\/ketang\/[\d]+\.html)"/', $list_body, $list_row_array)) {
            $row_num = count($list_row_array[1]);
        }
    ...
    echo $detail_url, "\n";
                ...
                //
                if($page_no == 1) {
                    if(preg_match('/<h1 class="wenzhang_tit">(.*)<\/h1>/isU', $detail_body, $detail_title_array)) {
                        $title = $detail_title_array[1];
                    }
                    if(preg_match('/<div class="xiaobian">([\d]{4}\-[\d]{2}\-[\d]{2} [\d]{2}:[\d]{2}:[\d]{2})/', $detail_body, $detail_time_array)) {
                        $pub_time = $detail_time_array[1];
                    }
                }
                if(preg_match('/<div class="v5">(.*)            <\/div>/isU', $detail_body, $detail_body_array)) {
                    $content .= $detail_body_array[1];
                }
                if(preg_match('/<span><a href="(\/[a-z]+\/[\d]+_[\d]+\.html)">下一页/isU', $detail_body, $detail_url_array)) {
        ...
        if(preg_match('/<span><a href="(\/[a-z]+\/[\d]+\/index_[\d]+\.html)">下一页/isU', $list_body, $list_next_array))    {
